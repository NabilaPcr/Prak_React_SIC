        ...
        <button
          ...
          className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition shadow-lg font-bold text-sm ${
            showForm
              ...
              : "bg-teks hover:bg-biru text-white"
          }`}
        >
          {showForm ? (
            <>
      ...
      <div className="p-6 space-y-6">
        {showForm && (
          <div className="bg-white p-6 rounded-3xl border-2 border-dashed border-garis animate-in fade-in zoom-in duration-300">
            <h3 className="text-lg font-bold mb-4 text-teks">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                className="p-3 bg-latar rounded-xl outline-none border border-transparent focus:border-biru"
              />
              <input
                type="email"
                placeholder="Email"
            ...
            <button className="mt-4 bg-biru text-white px-6 py-2 rounded-xl font-bold">
              Simpan Data
            </button>
          </div>
        )}
            ...
            <tbody className="divide-y divide-garis">
              {customersData.map((customer) => (
                <tr
                  ...
                  className="hover:bg-latar/30 transition-colors group"
                >
                  <td className="px-6 py-4 font-bold text-teks">
                    {customer.customerId}
                  </td>
                  ...
                  <td className="px-6 py-4 text-teks-samping text-sm">
                    {customer.email}
                  </td>
                  <td className="px-6 py-4 text-teks-samping text-sm">
                    {customer.phone}
                  ...
                  <td className="px-6 py-4 font-medium text-teks">
                    {getPointsByTier(customer.loyalty).toLocaleString("id-ID")}
                  </td>

                  <td className="px-6 py-4">
                    <span className="px-3 py-1 rounded-lg bg-green-100 text-green-700 text-xs font-bold">
                      {getDiscountByTier(customer.loyalty)}%
                    </span>
                  </td>

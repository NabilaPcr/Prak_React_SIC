...
export default function CustomerDetail() {
  ...
  const points = customer ? getPointsByTier(customer.loyalty) : 0;
  const discount = customer ? getDiscountByTier(customer.loyalty) : 0;
  const nextTier = customer ? getNextTier(customer.loyalty) : null;
  const nextTierPoints = customer ? getPointsToNextTier(customer.loyalty) : null;
  const remainingPoints = nextTierPoints && points ? nextTierPoints - points : 0;
              ...
              <div className="flex flex-col md:flex-row md:items-end -mt-12 mb-6">
                ...
                <div className="mt-4 md:mt-0 md:ml-4 flex-1">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <h2 className="text-2xl font-bold text-teks">
                    ...
                    <span
                      className={`px-4 py-2 rounded-xl text-sm font-bold ${getLoyaltyColor(customer.loyalty)}`}
                    >
                ...
                <div className="bg-latar/30 rounded-xl p-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-teks font-medium">{totalOrders}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-garis">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="bg-blue-50 rounded-xl p-4">
                    <p className="text-xs text-blue-600 font-bold uppercase">Current Points</p>
                  ...
                  <div className="bg-green-50 rounded-xl p-4">
                    <p className="text-xs text-green-600 font-bold uppercase">Discount</p>
                    <p className="text-2xl font-bold text-green-700">{discount}%</p>
                  </div>

                  {/* Member Tier */}
                  <div className="bg-orange-50 rounded-xl p-4">
                    <p className="text-xs text-orange-600 font-bold uppercase">Member Tier</p>
                    <p className="text-2xl font-bold text-orange-700">{customer.loyalty}</p>
                  </div>

                  {/* Next Tier */}
                  <div className="bg-purple-50 rounded-xl p-4">
                    <p className="text-xs text-purple-600 font-bold uppercase">Next Tier</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex justify-between py-2 border-b border-garis">
                    ...
                    <span className="text-teks font-medium">{lastOrder}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-garis">
